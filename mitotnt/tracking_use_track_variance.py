...
for frame in range(start_frame, end_frame-tracking_interval, tracking_interval):
    ...
    for i in range(number_m):
        ...
        elif distance_cutoff_mode == 2:
            if i not in track_coords_dict.keys():
                track = []
            else:
                track = track_coords_dict[i]
            if len(track) < 3:
                dist_cutoff = sorted(row)[cutoff_num_neighbor]
                # dist_cutoff = cutoff_speed * frame_interval # fixed distance cutoff of default 1 um/s * frame interval (s)

            else:
                disps = [np.linalg.norm((track[t]-track[t+1])) for t in range(len(track)-1)]
                dist_cutoff = (np.std(disps)) ** 2

        else:
            raise Exception('Distance cutoff mode must be either 1 or 2, not '+str(distance_cutoff_mode))
...
#     print('Number of filtered nodes is', len(filtered_nodes))
    ...
    print('Number of nodes at frame {}, {} are {}, {}'.format(frame, frame+tracking_interval, number_m, number_n),
          ...
          '\nTracking takes {:.2f} s'.format(end - start))
    ...
    # make a dictionary for easy access to the coords of tracks
    track_coords_dict = {t[1][-1]:t[4] for t in ongoing_tracks}
